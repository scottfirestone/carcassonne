const dom = require('./domInventory').dom;
const TileInventory = require('./tileInventory');
...
class Game {
  ...
  revealPlayerNameInputs(num) {
    for (let i = 1; i <= num; i++) {
      dom(`inputPlayerName${i}`).css('display', 'block');
    }
    for (let i = 5; i > num; i--) {
      dom(`inputPlayerName${i}`).hide();
    }
  }
  ...
  updateTileCount() {
    ...
    if (tilesLeft === 0) {
      dom('buttonPlaceTile').off('click', this.placeTile);
      dom('buttonPlaceTile').on('click', this.endGame);
    }
  }
  ...
  showPlaceTileButton() {
    dom('buttonPlaceTile').show();
    dom('buttonPlaceMeeple').hide();
    dom('buttonSkipMeeple').hide();
  }

  showPlaceMeepleButtons() {
    dom('buttonPlaceTile').hide();
    dom('buttonPlaceMeeple').show();
    dom('buttonSkipMeeple').show();
  }
  ...
  placeMeeple(e) {
    ...
    if (inXRange && inYRange) {
      ...
      dom('gameBoard').append(this.currentMeeple.dom);
