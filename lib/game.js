...
class Game {
  ...
  prepare() {
    this.sectionShow('section-pre-game');
  }

  newGameButtonClicked(e) {
    this.sectionShow('section-player-input');
  ...
  beginGameButtonClicked(e) {
    this.domItems.buttonNewGame.off('click');
    this.sectionShow('section-game');
  }

  setup() {
    // Define all existing DOM jquery objects
    this.domItems = {
      buttonNewGame: $('#btn-new-game'),
    ...
    this.animations.sectionShow = new TimelineMax({
      paused: true,
      onComplete: this.animationCompleted,
      onCompleteParams: [ this, { type: 'sectionShow' }]
    });
    // this.animations.newGame = new TimelineMax({
    //   paused: true,
    //   onComplete: this.animationCompleted,
    //   onCompleteParams: [ { type: 'newGame' } ]
    // })
    //   .to(this.domItems.buttonNewGame, 0.2, { autoAlpha: 0 })
    //   .to(this.domItems.butt);
  }
  ...
  sectionShow(id) {
    ...
    this.animations.sectionShow.clear();
  ...
  animationCompleted(game, info) {
    switch (info.type) {
      case 'sectionShow':
        if (game.sectionId === 'section-pre-game') {
          game.domItems.buttonNewGame.on('click', game.newGameButtonClicked.bind(game));
        }
        else if (game.sectionId === 'section-player-input') {
          game.domItems.buttonBeginGame.on('click', game.beginGameButtonClicked.bind(game));
        }
        else if (game.sectionId === 'section-game') {
          game.start();
        }
        break;
      case 'newGame':
